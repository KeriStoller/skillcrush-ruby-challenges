...
def birthday
	puts "What is your birthdate?  Note, it must be in the MMDDYYYY method."
	birthdate = gets
end

def birth_path_number
	birthdate = birthday
	number = (birthdate[0].to_i + birthdate[1].to_i + birthdate[2].to_i + birthdate[3].to_i + birthdate[4].to_i + birthdate[5].to_i + birthdate[6].to_i + birthdate[7].to_i).to_s
...
def numerology_message
	birth_path_num = birth_path_number
case birth_path_num
	when 1
	...
	when 3
		message = "Number Three is a sociable, friendly, and outgoing vibration.  Kind, poistive, and optimistic, Three's enjoy life and have a good sense of humor.  Ruled by Jupiter."
	when 4
	...
	when 9
		...
		puts "Better try again, your birthdate isn't a number between 1 & 9."
	end
 
 puts "Your birth path number is #{birth_path_num}. #{message}"
 end
